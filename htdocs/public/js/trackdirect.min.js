...
this._map.showHideMarkers();},toggleCbAprsPositions:function(){if(this.isCbAprsMarkersVisible()){this._map.state.isCbAprsMarkersVisible=false;}else{this._map.state.isCbAprsMarkersVisible=true;}
this._map.showHideMarkers();},toggleOgnPositions:function(){if(this.isOgnMarkersVisible()){this._map.state.isOgnMarkersVisible=false;}else{this._map.state.isOgnMarkersVisible=true;}
...
var data={center:this._map.getCenterLiteral(),zoom:this._map.getZoom(),};this._emitEventListeners("position-request-sent",data);}},_sendIdleRequest:function(){if(this._websocket.isPositionRequestSent()){this._websocket.doSendNewPositionRequest(0,0,0,0,this._map.state.getTimeLength()/60,this.getTimeTravelTimestamp(),false);}},_deactivateFiltered:function(sendNewRequest){this._map.resetAllMarkers();if(this._map.state.openInfoWindow!==null){}
this._map.deactivateFilteredMode();this.stopTrackStation();this._emitEventListeners("filter-changed",[]);this._emitEventListeners("filter-stopped");if(sendNewRequest){trackdirect.services.callbackExecutor.add(this,this._sendPositionRequest,[]);}
...
sizeStrValue+
scaleStrValue+
".svg";return trackdirect.settings.baseUrl+url;},_getIconFilePathCoursePart:function(course){var courseStrValue="";if(course!==null){var courseValue=Math.round(parseInt(course)/10)*10;while(courseValue>360){courseValue=courseValue-360;}
...
"transmit"+
index+
".svg",size:new google.maps.Size(60,60),origin:new google.maps.Point(0,0),anchor:new google.maps.Point(30,30),};}else if(typeof L==="object"){var icon=L.icon({iconUrl:trackdirect.settings.baseUrl+
trackdirect.settings.imagesBaseDir+
"transmit"+
index+
".svg",iconSize:[60,60],iconAnchor:[30,30],});}
...
return rawPath;}
return null;};trackdirect.models.Packet.prototype.getPacketTypeName=function(){if(typeof this.packet_type_id!=="undefined"&&this.packet_type_id!==null&&this.packet_type_id!==""){switch(parseInt(this.packet_type_id)){case 1:return'Position';break;case 2:return'Direction';break;case 3:return'Weather';break;case 4:return'Object';break;case 5:return'Item';break;case 6:return'Telemetry';break;case 7:return'Message';break;case 8:return'Query';break;case 9:return'Response';break;case 10:return'Status';break;case 11:return'Other';break;default:return'Unknown';break;}}};
...
"dotColor"+
colorId+
".svg",size:new google.maps.Size(12,12),origin:new google.maps.Point(0,0),anchor:new google.maps.Point(6,6),};}else if(typeof L==="object"){var icon=L.icon({iconUrl:trackdirect.settings.baseUrl+
trackdirect.settings.imagesBaseDir+
"dotColor"+
colorId+
".svg",iconSize:[12,12],iconAnchor:[6,6],popupAnchor:[-3,-12],});}
...
"dotColor"+
colorId+
".svg";var scaledImageSize=new google.maps.Size(12,12);var imageSize=new google.maps.Size(12,12);var imageAnchor=new google.maps.Point(6,6);var opacity=1.0;}else{var scalePx=24;var sizePx=24;if(this._shouldMarkerSymbolBeScaled()){scalePx=48;}
...
"dotColor"+
colorId+
".svg";var icon=L.icon({iconUrl:iconUrl,iconSize:[12,12],iconAnchor:[6,6],});var opacity=0.8;}else{var scalePx=24;var sizePx=24;if(this._shouldMarkerSymbolBeScaled()){scalePx=48;}
if(this.packet.course!==null){sizePx=sizePx+7;if(scalePx<sizePx){scalePx=sizePx;}}
...
trackdirect.models.MapState=function(){this._init();};trackdirect.models.MapState.prototype._init=function(){this.isFilterMode=false;this.filterStationIds=[];this.isGhostMarkersVisible=false;this.isCwopMarkersVisible=true;this.isCbAprsMarkersVisible=true;this.isOgnMarkersVisible=true;this.isStationaryMarkersVisible=true;this.isInternetMarkersVisible=true;this.isUnknownMarkersVisible=true;this.isOgflymMarkersVisible=true;this.visibleSymbols=[];this.useImperialUnit=false;this.showPHGCircles=0;this.showRNGCircles=0;this.trackStationId=null;this.onlyTrackRecentPackets=false;this.timeLengthInSeconds=3600;this.endTimeTravelTimestamp=null;this.currentMarkerZindex=200;this.serverClientTimestampDiff=0;this.openInfoWindow=null;this.openInfoWindowForMarkerIdKey=null;};trackdirect.models.MapState.prototype.getOldestAllowedPacketTimestamp=function(){var oldestAllowedTimestamp=Math.floor(Date.now()/1000)-this.timeLengthInSeconds;if(this.endTimeTravelTimestamp!==null){oldestAllowedTimestamp=this.endTimeTravelTimestamp-this.timeLengthInSeconds;}
...
weatherDateString=moment(weatherDate).format(trackdirect.settings.dateFormatNoTimeZone);var weatherData=$(document.createElement("fieldset"));weatherData.append("<legend><a href='' onclick='loadView(\"/views/weather.php?id="+this._marker.packet.station_id+"&imperialUnits="+(this._defaultMap.state.useImperialUnit?"1":"0")+"\");return false;'>Latest Weather</a> -  "+weatherDateString+"</legend>");if(!trackdirect.isMobile&&$(window).height()>=300){weatherData.append(this._getWeatherDivTemperatureString());weatherData.append(this._getWeatherDivHumidityString());weatherData.append(this._getWeatherDivPressureString());weatherData.append(this._getWeatherDivRainString());weatherData.append(this._getWeatherDivWindString());}
weatherDiv.append(weatherData);return weatherDiv;};trackdirect.models.InfoWindow.prototype._getTelemetryDiv=function(){var telemetryDiv=$(document.createElement("div"));telemetryDiv.css("clear","both");telemetryDiv.css("color","#823030");var telemetryDate=new Date(this._marker.packet.latest_telemetry_packet_timestamp*1000);telemetryDateString=moment(telemetryDate).format(trackdirect.settings.dateFormatNoTimeZone);telemetryDiv.append("<a href='' onclick='loadView(\"/views/telemetry.php?id="+this._marker.packet.station_id+"&imperialUnits="+(this._defaultMap.state.useImperialUnit?"1":"0")+"\");return false;'>Latest Telemetry</a>  "+telemetryDateString+"<br/>");return telemetryDiv;};trackdirect.models.InfoWindow.prototype._getPacketCommentDiv=function(){var comment="";var ognSummary=escapeHtml(this._marker.packet.getOgnSummary());if(ognSummary!=""){comment=ognSummary;}else if(typeof this._marker.packet.comment!=="undefined"&&this._marker.packet.comment!==null){comment=Autolinker.link(escapeHtml(this._marker.packet.comment),{newWindow:true,});}
